...
2. 单元边界只来自原文已有换行，或程序允许的冒号后断点；双星号、单星号、删除线、反引号、数学公式等特殊格式定界符内部的冒号绝不构成断点。没有边界的单元绝对不允许拆分、分段或分层。每个计划块用 start 和 end 表示一个连续、闭合的单元范围；范围必须按顺序、不得重叠，并覆盖每个单元且只覆盖一次。
...
9. 识别缩进表达的第二层列表；有序列表可使用 1.、3)、五、或 ① 等原文标记，无序列表可使用实心、空心或短横线标记。原文已有的序号和分点标记必须逐字符保留，不得重新编号，也不得增加、删除或重排项目。
